...
export default function SeekerApplyForm({ currentUser, onProfileLoaded }) {
        ...
        seekersApi.getByUserId(currentUser.id)
            .then(data => {
                ...
                setForm({
                    ...
                    skills,
                    phone:        data.phone        || "",
                    // Берём email из анкеты, если есть — иначе из аккаунта
    ...
    const clear = () => { setForm({ ...empty, email: currentUser?.email || "" }); setSavedOk(false); };
    ...
    const submit = async () => {
        ...
        const body = {
            ...
            email:          form.email || null,   // ← сохраняем email из формы
        };
        try {
